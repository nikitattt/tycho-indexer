...
use super::fixed_pools::{fixed_pool, FixedPool};
use crate::{
    abi::pool::events::{
...
pub fn map_events(block: eth::Block) -> Result<Events, anyhow::Error> {
    let mut pool_events = block
        ...
        .flat_map(|tx| {
            ...
            receipt
                ...
                .iter()
                .filter_map(|log| {
                    fixed_pool(log.address.as_slice()).and_then(|pool| log_to_event(log, pool, &tx))
                })
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
...
fn log_to_event(event: &Log, pool: &FixedPool, tx: &TransactionTrace) -> Option<PoolEvent> {
    if let Some(init) = Initialize::match_and_decode(event) {
        Some(PoolEvent {
            log_ordinal: event.ordinal,
            pool_address: Hex(pool.address.to_vec()).to_string(),
            token0: Hex(pool.token0.to_vec()).to_string(),
            token1: Hex(pool.token1.to_vec()).to_string(),
