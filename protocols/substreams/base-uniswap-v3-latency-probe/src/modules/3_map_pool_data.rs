...
pub fn map_pool_data(block: eth::Block) -> Result<BlockPoolData, anyhow::Error> {
    collect_pool_data(block, |address| fixed_pool(address).map(|pool| pool.to_pool()))
}
