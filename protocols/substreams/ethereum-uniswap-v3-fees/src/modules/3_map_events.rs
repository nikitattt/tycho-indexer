...
const FLASH_TOPIC: [u8; 32] =
    hex!("bdbdb71d7860376ba52b25a5028beea23581364a40522f6bcfb86bb1f2dca633");
...
mod tests {
    ...
    fn recognizes_only_supported_v3_pool_event_shapes() {
        ...
        for (topic, topic_count, data_len, event_kind) in cases {
            ...
            assert!(match event_kind {
                EventKind::Initialize => Initialize::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::Swap => Swap::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::Flash => Flash::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::Mint => Mint::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::Burn => Burn::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::Collect => Collect::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::SetFeeProtocol => SetFeeProtocol::match_log(&event_log(topic, topic_count, data_len)),
                EventKind::CollectProtocol => CollectProtocol::match_log(&event_log(topic, topic_count, data_len)),
            }, "{event_kind:?} topic must match the generated ABI matcher");
        }
